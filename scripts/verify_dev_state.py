...
for sch in ("retail_bronze", "retail_silver", "retail_gold", "retail_ml"):
...
show("""SELECT 'pl_applications_bronze' AS tbl, COUNT(*) FROM asb_dev.retail_bronze.pl_applications_bronze
       UNION ALL SELECT 'pl_facilities_bronze', COUNT(*) FROM asb_dev.retail_bronze.pl_facilities_bronze
       UNION ALL SELECT 'pl_credit_performance_bronze', COUNT(*) FROM asb_dev.retail_bronze.pl_credit_performance_bronze
       UNION ALL SELECT 'pl_sas_final_scores_bronze', COUNT(*) FROM asb_dev.retail_bronze.pl_sas_final_scores_bronze""")
...
show("""SELECT 'pl_applications_silver' AS tbl,
              ...
              COUNT(*) AS total_rows
       FROM asb_dev.retail_silver.pl_applications_silver
       UNION ALL SELECT 'pl_facilities_silver',
              SUM(CASE WHEN _is_current THEN 1 ELSE 0 END), COUNT(*)
       FROM asb_dev.retail_silver.pl_facilities_silver
       UNION ALL SELECT 'pl_sas_final_scores_silver',
              SUM(CASE WHEN _is_current THEN 1 ELSE 0 END), COUNT(*)
       FROM asb_dev.retail_silver.pl_sas_final_scores_silver
       UNION ALL SELECT 'pl_credit_performance_silver',
              NULL, COUNT(*)
       FROM asb_dev.retail_silver.pl_credit_performance_silver""")
...
show("""SELECT _population, target_flag, COUNT(*) AS n
       FROM asb_dev.retail_gold.pl_scorecard_dev_data
       GROUP BY 1, 2 ORDER BY 1, 2""")
...
show("""SELECT feature, var_kind, ROUND(SUM(iv), 4) AS total_iv,
       ...
       FROM asb_dev.retail_gold.pl_woe_iv
...
show("""SELECT population, metric, ROUND(value, 4) AS value, alert_level
       ...
       WHERE run_timestamp = (SELECT MAX(run_timestamp) FROM asb_dev.retail_ml.pl_monitoring_log)
